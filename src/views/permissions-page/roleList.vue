<template>
  <div class="home-box">
    <el-form v-show="showSearch" :inline="true" :model="searchForm">
      ...
      <el-form-item>
        <el-button v-auth="'/adminAuth/getRoleList'" type="primary" style="margin-left: -16px" @click="searchEvent">
          查询
        </el-button>
      </el-form-item>
    </el-form>

    <right-toolbar v-model:showSearch="showSearch" :have-import="false" @queryTable="getListData"
      @onOutClick="onDownTemplate" @onAddClick="onDownTemplate"></right-toolbar>
    <el-card>
      <el-table v-loading="loading" :data="tableData" element-loading-text="加载中..." border>
        ...
        <el-table-column prop="address" label="操作" min-width="160">
          <template #default="{ row }">
            <el-button v-auth="'/adminAuth/setting'" type="primary" size="small" @click="handleEdit(row)">
              配置权限
            </el-button>
          </template>
        </el-table-column>
      ...
      <pagination v-show="total > 0" v-model:page="searchForm.page" v-model:limit="searchForm.page_size" :total="total"
